...
def exe(word_vectors_file, vector_preloaded_path, train_path, dev_path, test_path, hidden_sizes, maxlen, pep, fep, ppat, fpat, plr, flr):
    ...
    combined = LSTM_CNN(word_vectors, hidden_sizes=hidden_sizes, epochs=fep, lstm_params=lstm_params, patience=fpat, learning_rate=flr)
...
parser.add_argument('--fpat', type=int, default=20)
parser.add_argument('--plr', type=float, default=0.0001)
parser.add_argument('--flr', type=float, default=0.1)


args = parser.parse_args()

exe(args.vectors, args.plvec, args.train, args.dev, args.test, args.hs, args.max, args.pep, args.fep, args.ppat, args.fpat, args.plr, args.flr)
...
def exe(word_vectors_file, vector_preloaded_path, train_path, dev_path, test_path, hidden_sizes, maxlen, pep, fep, ppat, fpat):
    ...
    lstm = Model(word_vectors, hidden_sizes=hidden_sizes, epochs=pep, patience=ppat)
    lstm_params = lstm.train(train, dev, test, maxlen)
    combined = LSTM_CNN(word_vectors, hidden_sizes=hidden_sizes, epochs=fep, lstm_params=lstm_params, patience=fpat)
...
exe(args.vectors, args.plvec, args.train, args.dev, args.test, args.hs, args.max, args.pep, args.fep, args.ppat, args.fpat)